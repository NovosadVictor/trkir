...
def create(dirname, subdirname):
    if not os.path.isdir(os.path.join('check', dirname, 'xlses', 'data')):
        os.mkdir(os.path.join('check', dirname, 'xlses', 'data'))

    xlses = [f for f in os.listdir(os.path.join('check', dirname, 'xlses', 'cleared')) if subdirname in f]

    mouses_info = [[[] for i in range(7)], [[] for i in range(7)]]

    dates = [[], []]

    with open(os.path.join('check', dirname, 'xlses', 'cleared', xlses[0]), newline='') as csvfile:
        data = list(csv.reader(csvfile))
        if len(data) > 20:
            data = np.array(data).T.tolist()

    mouses_info = [[data[7 * m + i] for i in range(7)] for m in range(2)]
    dates = data[-2:]

    # for xls in xlses:
    #
    #     sheet = xlrd.open_workbook(os.path.join('check', dirname, 'xlses', 'cleared', xls))
    ...
    for m in range(2):
       ...
       #     print(dates[1])
        ...
        with open(os.path.join('check', dirname, 'xlses', 'data', '{}_{}.csv'.format(m, subdirname)), 'w') as csvfile:
