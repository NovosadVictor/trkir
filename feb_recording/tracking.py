...
def detect_for_dir(dirname):
 ...
 #   os.mkdir('/storage/Data/ucsf_processed/{}'.format(dirname.split('/')[-1]))
    save_url = '/storage/Data/pc_processed/'
    ...
    for url_i, url in enumerate(urls):
        ...
        mouse_data = detect_mouse_data_from_url(url)

        true_mouse_data = [[], [], [], []]
        
        try:
            if len(mouse_data) != 4:
                raise Exception('!=4 mice')
            for m in range(4):
                mouse_center = np.array(mouse_data[m][2])
                if mouse_center[0] < cage_center[0]:
                    if mouse_center[1] < cage_center[1]:
                        true_mouse_data[0] = mouse_data[m]
                    else:
                        true_mouse_data[2] = mouse_data[m]
                else:
                    if mouse_center[1] < cage_center[1]:
                        true_mouse_data[1] = mouse_data[m]
                    else:
                        true_mouse_data[3] = mouse_data[m]

            mouse_data = true_mouse_data[:]
        except:
            print('excepted', url)
            mouse_data = [[], [], [], []]
        full_data.append(mouse_data)

        draw_body_parts(img_url=url, drawing_parts=mouse_data, save_url=strange_url + url.split('/')[-1])
